...
// Semantic 타입별 유틸리티 클래스 매핑
const SEMANTIC_CLASS_MAP = {
...
// Semantic CSS 변수에서 유틸리티 클래스만 반환하는 함수
export const getSemanticUtilityClass = (cssVariable: string): string | null => {
  ...
  const classGenerator = SEMANTIC_CLASS_MAP[type as keyof typeof SEMANTIC_CLASS_MAP];
...
export const getPrimitiveValue = (cssVariable: string): string | null => {
  ...
  if (primitiveColorName && primitiveColors[primitiveColorName] && shade) {
    return primitiveColors[primitiveColorName][shade as keyof typeof primitiveColors[typeof primitiveColorName]] || null;
  }
